...
class InventoryService:
    ...
    def _get_or_create_inventory(
        ...
        if batch is not None:
            inv = self.inv_repo.get_by_product_warehouse_and_batch(product_id, warehouse_id, batch)
            if not inv:
                inv = Inventory(
                    product_id=product_id,
                    warehouse_id=warehouse_id,
        ...
        all_records = Inventory.query.filter(
            Inventory.product_id == product_id,
            Inventory.warehouse_id == warehouse_id,
        ).order_by(Inventory.quantity_on_hand.desc()).all()
        if all_records:
    ...
    def _calculate_average_cost(self, product_id: int, warehouse_id: int) -> float:
        ...
        result = InventoryLedger.query.with_entities(
            func.sum(InventoryLedger.quantity * InventoryLedger.unit_cost),
            func.sum(InventoryLedger.quantity),
        ).filter(
    ...
    def deduct_stock(
        ...
        batch = batch_number if batch_number else None
        inv = self._get_or_create_inventory(product_id, warehouse_id, batch)
        current_qty = float(inv.quantity_on_hand or 0)
