...
export default function GIVListPage() {
  ...
  const [approveTarget, setApproveTarget] = useState<GoodsIssueVoucher | null>(null);
  const location = useLocation();
  const prefilledOrderId = (location.state as any)?.orderId;
  const [data, setData] = useState<GoodsIssueVoucher[]>([]);
  ...
  const handleViewDetail = async (giv: GoodsIssueVoucher) => {
    try {
      const full = await warehousesApi.getGiv(giv.id);
      setDetailGIV(full);
    } catch {
      setDetailGIV(giv);
    }
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await warehousesApi.createGiv({
        ...form, voucher_number: `GIV-${Date.now()}`,
        items: lineItems.map((l) => ({
          product_id: l.product_id, quantity: l.quantity, batch_number: l.batch_number || undefined,
        })),
      });
      blurActiveElement();
      setFormOpen(false);
      ...
      alert(err?.response?.data?.error || err?.response?.data?.detail || err?.message || 'Failed to create GIV');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleApprove = async () => {
    ...
    try {
      await warehousesApi.approveGiv(approveTarget.id);
      blurActiveElement();
      setApproveTarget(null);
      fetch();
    } catch (err: any) {
      alert(err?.response?.data?.message || err?.response?.data?.error || 'Failed to approve GIV');
    }
  };
  ...
  const columns = [
    ...
    { id: 'issued_by_name', label: 'Issued By' },
    {
      ...
      render: (row: GoodsIssueVoucher) => <StatusChip status={row.status} />,
    },
    {
      id: 'actions', label: 'Actions', width: 160,
      render: (row: GoodsIssueVoucher) => (
  ...
  return (
    <>
      <Box sx={{ mb: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 2 }}>
        <Box>
          <Typography variant="h4">Goods Issue Vouchers</Typography>
      ...
      <Dialog open={formOpen} onClose={() => { blurActiveElement(); setFormOpen(false); setLineItems([]); }} maxWidth="md" fullWidth disableRestoreFocus>
        <Box component="form" onSubmit={handleCreate}>
          <DialogTitle>New Goods Issue Voucher</DialogTitle>
          <DialogContent>
                ...
                {lineItems.map((item, idx) => (
                  <Box key={idx} sx={{ display: 'flex', gap: 2, mb: 1.5, alignItems: 'center', flexWrap: 'wrap' }}>
                    <TextField select label="Product" size="small" sx={{ flex: 2, minWidth: 180 }}
                      ...
                      onChange={(e) => {
                          updateLineItem(idx, 'product_id', Number(e.target.value));
                      }}>
      ...
      <Dialog open={!!approveTarget} onClose={() => { blurActiveElement(); setApproveTarget(null); }} disableRestoreFocus>
        ...
        <DialogContent>
          <DialogContentText>
            Are you sure you want to approve GIV <strong>{approveTarget?.voucher_number}</strong>?
            This will deduct the issued quantities from inventory.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
      ...
      <Dialog open={detailOpen} onClose={() => { blurActiveElement(); setDetailOpen(false); }} maxWidth="sm" fullWidth disableRestoreFocus>
        <DialogTitle sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span>{detailGIV?.voucher_number}</span>
          <IconButton size="small" onClick={() => setDetailOpen(false)}><CloseIcon /></IconButton>
        </DialogTitle>
        <DialogContent>
          <Box sx={{ mb: 2 }}>
            <Typography variant="body2" color="text.secondary">Warehouse: {detailGIV?.warehouse_name}</Typography>
            <Typography variant="body2" color="text.secondary">Date: {detailGIV?.voucher_date}</Typography>
            <Typography variant="body2" color="text.secondary">Status: {detailGIV?.status}</Typography>
            {detailGIV?.created_by_name && <Typography variant="body2" color="text.secondary">Created By: {detailGIV.created_by_name}</Typography>}
            {detailGIV?.issued_by_name && <Typography variant="body2" color="text.secondary">Issued By: {detailGIV.issued_by_name}</Typography>}
            {detailGIV?.notes && <Typography variant="body2" color="text.secondary">Notes: {detailGIV.notes}</Typography>}
          </Box>
          <Typography variant="subtitle2" sx={{ mb: 1 }}>Items</Typography>
          {detailGIV?.items && detailGIV.items.length > 0 ? (
            <TableContainer component={Paper} variant="outlined">
              <Table size="small">
                ...
                <TableBody>
                  {detailGIV.items.map((item: GIVItem) => (
