...
export default function GRVListPage() {
  ...
  const [approveTarget, setApproveTarget] = useState<GoodsReceiveVoucher | null>(null);
  ...
  const fetch = useCallback(async () => {
    ...
    try {
      ...
      const res = await warehousesApi.grvList(params);
      setData(res.items);
      setTotal(res.total);
    } finally {
      setLoading(false);
  ...
  const handleViewDetail = async (grv: GoodsReceiveVoucher) => {
    try {
      const full = await warehousesApi.getGrv(grv.id);
      setDetailGRV(full);
    } catch {
      setDetailGRV(grv);
    }
  ...
  const handleApprove = async () => {
    ...
    try {
      await warehousesApi.approveGrv(approveTarget.id);
      blurActiveElement();
      setApproveTarget(null);
      fetch();
    } catch (err: any) {
      alert(err?.response?.data?.message || err?.response?.data?.error || 'Failed to approve GRV');
    }
  };

  const addLineItem = () => {
    setLineItems([...lineItems, { product_id: 0, product_name: '', quantity: 1, unit_cost: 0 }]);
  };
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.warehouse_id || lineItems.length === 0 || !lineItems.some((l) => l.product_id)) {
      alert('Please fill warehouse and at least one product item');
      return;
    }
    setSubmitting(true);
    try {
      await warehousesApi.createGrv({
        ...form, voucher_number: `GRV-${Date.now()}`,
      ...
      alert(err?.response?.data?.error || err?.response?.data?.detail || err?.message || 'Failed to create GRV');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const columns = [
    ...
    { id: 'received_by_name', label: 'Received By' },
    {
      id: 'status', label: 'Status',
      render: (row: GoodsReceiveVoucher) => <StatusChip status={row.status} />,
    },
    {
      id: 'actions', label: 'Actions', width: 160,
      render: (row: GoodsReceiveVoucher) => (
  ...
  return (
    <>
      <Box sx={{ mb: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 2 }}>
        <Box>
          <Typography variant="h4">Goods Receive Vouchers</Typography>
          <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>
            Record incoming goods to warehouse
          </Typography>
        </Box>
        {canCreate && (
      ...
      <Dialog open={formOpen} onClose={() => { blurActiveElement(); setFormOpen(false); setLineItems([]); }} maxWidth="md" fullWidth disableRestoreFocus>
        <Box component="form" onSubmit={handleCreate}>
          <DialogTitle>New Goods Receive Voucher</DialogTitle>
          <DialogContent>
            <Grid container spacing={2.5} sx={{ pt: 2 }}>
              ...
              <Grid size={{ xs: 12, sm: 6 }}>
                <TextField label="Reference Type" select fullWidth value={form.reference_type}
      ...
      <Dialog open={!!approveTarget} onClose={() => { blurActiveElement(); setApproveTarget(null); }} disableRestoreFocus>
        ...
        <DialogContent>
          <DialogContentText>
            Are you sure you want to approve GRV <strong>{approveTarget?.voucher_number}</strong>?
            This will update inventory with the received quantities.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
      ...
      <Dialog open={detailOpen} onClose={() => { blurActiveElement(); setDetailOpen(false); }} maxWidth="sm" fullWidth disableRestoreFocus>
        <DialogTitle sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span>{detailGRV?.voucher_number}</span>
          <IconButton size="small" onClick={() => setDetailOpen(false)}><CloseIcon /></IconButton>
        </DialogTitle>
        <DialogContent>
          <Box sx={{ mb: 2 }}>
            <Typography variant="body2" color="text.secondary">Warehouse: {detailGRV?.warehouse_name}</Typography>
            <Typography variant="body2" color="text.secondary">Date: {detailGRV?.voucher_date}</Typography>
            <Typography variant="body2" color="text.secondary">Status: {detailGRV?.status}</Typography>
            {detailGRV?.created_by_name && <Typography variant="body2" color="text.secondary">Created By: {detailGRV.created_by_name}</Typography>}
            {detailGRV?.received_by_name && <Typography variant="body2" color="text.secondary">Received By: {detailGRV.received_by_name}</Typography>}
            {detailGRV?.notes && <Typography variant="body2" color="text.secondary">Notes: {detailGRV.notes}</Typography>}
          </Box>
          <Typography variant="subtitle2" sx={{ mb: 1 }}>Items</Typography>
          {detailGRV?.items && detailGRV.items.length > 0 ? (
            <TableContainer component={Paper} variant="outlined">
              <Table size="small">
                ...
                <TableBody>
                  {detailGRV.items.map((item: GRVItem) => (
                    <TableRow key={item.id} hover>
                      <TableCell>{item.product_name}</TableCell>
                      <TableCell align="right">{item.quantity}</TableCell>
                      <TableCell align="right">{item.unit_cost ? formatCurrency(item.unit_cost) : '-'}</TableCell>
                      <TableCell align="right">
                        {item.unit_cost ? formatCurrency(item.quantity * item.unit_cost) : '-'}
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
